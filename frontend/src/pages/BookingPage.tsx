...
function BookingPage() {
  ...
  useEffect(() => {
    ...
    api
      .get("/public/initialization-status")
      .then((response) => {
  ...
  const totalAmount =
    formData.adultTickets * adultPrice + formData.studentTickets * studentPrice;
  ...
  const handleTicketChange = (type: "adult" | "student", delta: number) => {
    const currentValue =
      type === "adult" ? formData.adultTickets : formData.studentTickets;
    const newValue = Math.max(
      0,
      Math.min(
        maxTickets -
          (type === "adult" ? formData.studentTickets : formData.adultTickets),
        currentValue + delta
      )
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 2) {
      ...
      try {
        ...
        navigate(
          `/booking/success/${response.data.bookingReference}/${response.data.email}`
        );
      } catch (error) {
        ...
        {step === 1 && (
          <div className="booking-step">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                if (formData.showId) handleTimeSelection(formData.showId);
              }}
            >
                ...
                showsByDate.map((dateGroup) => (
                  <div key={dateGroup.date} className="date-group">
                    <h3 className="date-header">
                      {t("common.date")}: {dateGroup.formattedDate}
                    </h3>
                      ...
                      {dateGroup.shows.map((show) => (
                        <div key={show.id} className="time-option">
                          <input
                            ...
                            onChange={() =>
                              setFormData({ ...formData, showId: show.id })
                            }
                            required
                          />
                          <label
                            htmlFor={`show_${show.id}`}
          ...
          <div className="booking-step">
            ...
            <div className="selected-time">
              <p>
                ...
                {selectedShow.date ? formatDate(selectedShow.date) : ""}
              </p>
              <p>
            ...
            <form onSubmit={handleSubmit}>
              <div className="ticket-selection">
                <div className="ticket-type">
                  <h3>
                    {settings.adultLabel || t("home.adultTicket")} ({adultPrice}{" "}
                    {t("common.kr")})
                  </h3>
                ...
                <div className="ticket-type">
                  <h3>
                    {settings.studentLabel || t("home.studentTicket")} (
                    {studentPrice} {t("common.kr")})
                  </h3>
                  <div className="ticket-counter">
                    <button
                      type="button"
              ...
              <div className="total-section">
                <div className="total-display">
                  <p>
                    <strong>{t("common.totalToPay")}:</strong>{" "}
          ...
          <div className="booking-step">
            ...
            <div className="booking-summary">
              ...
              <ul>
                ...
                <li>
                  <strong>{t("common.time")}:</strong> {selectedShow.startTime}-
                  {selectedShow.endTime}
                </li>
                <li>
                  <strong>
                    {settings.adultLabel || t("home.adultTicket")}:
                  </strong>{" "}
                  {formData.adultTickets} st
                </li>
                <li>
                  <strong>
                    {settings.studentLabel || t("home.studentTicket")}:
                  </strong>{" "}
                  {formData.studentTickets} st
                </li>
                <li>
                  <strong>{t("common.total")}:</strong> {totalAmount}{" "}
                  {t("common.kr")}
                </li>
              </ul>
            </div>
